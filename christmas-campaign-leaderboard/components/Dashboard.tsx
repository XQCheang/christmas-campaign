...
import { getStoredData, syncApplicantCount, addManualApplicants, updateRecruiterName, addNewRecruiter } from '../services/storage';
...
export const Dashboard: React.FC<DashboardProps> = ({ currentUser, isAdmin, onRefreshData }) => {
  ...
  // Determine which data to show on chart
  const chartData = useMemo(() => {
    const dailyData = Array.from({ length: 21 }, (_, i) => ({
      day: `Dec ${i + 1}`,
      dateNum: i + 1,
      count: 0
    }));
    ...
    sourceRecruiters.forEach(recruiter => {
        recruiter.applicants.forEach(app => {
            ...
            if (d.getFullYear() === currentYear && d.getMonth() === 11) { // Dec
                const day = d.getDate();
                if (day >= 1 && day <= 21) {
    ...
    return dailyData;
  }, [currentUser, allRecruiters, isAdmin]);
  ...
  // Sync name field when editor changes
  useEffect(() => {
  ...
  const handleUpdateScore = () => {
      if (!editingRecruiter && modalType === 'UPDATE') return;
        ...
        <div className="lg:col-span-2 bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-xl flex flex-col">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-xs text-slate-400">Dec 1 - Dec 21</p>
                </div>
            </div>
            ...
            <div className="bg-slate-800 rounded-2xl max-w-lg w-full border border-slate-700 shadow-2xl overflow-hidden flex flex-col">
                <div className="p-6 border-b border-slate-700 bg-slate-800/50 flex justify-between items-center">
                    <h3 className="text-xl font-bold text-white">
                        {modalType === 'CREATE' ? 'Add New Participant' : 'Update Recruiter'}
                    </h3>
